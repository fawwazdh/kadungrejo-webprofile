...
export default async function HalamanDetailBerita({ params }: Props) {
  ...
  // 1. Tarik detail berita yang sedang dibuka
  const queryBerita = `*[_type == "berita" && _id == $id][0]{
    _id, judul, tanggal, penulis, dilihat, konten, "gambarUrl": gambar.asset->url
  }`;

  // 2. Tarik 5 berita terbaru (TIDAK termasuk berita yang sedang dibaca saat ini)
  const queryTerbaru = `*[_type == "berita" && _id != $id] | order(tanggal desc)[0...5]{
    _id, judul, tanggal, dilihat, "gambarUrl": gambar.asset->url
  }`;

  // Eksekusi kedua query bersamaan
  const [berita, beritaTerbaru] = await Promise.all([
    client.fetch<DetailBerita>(queryBerita, { id }),
    client.fetch<BeritaSidebar[]>(queryTerbaru, { id }),
  ]);
    ...
    <div className="bg-sage-50 min-h-screen py-10 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <article className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-sage-200 p-6 md:p-10">
          ...
          {/* Meta Data (Tanggal, Penulis, Dilihat) */}
          ...
          {/* Isi Konten Menggunakan Portable Text Sanity */}
          {berita.konten ? (
        ...
        <aside className="lg:col-span-1">
          {/* Sticky membuat sidebar tetap terlihat saat di-scroll ke bawah */}
